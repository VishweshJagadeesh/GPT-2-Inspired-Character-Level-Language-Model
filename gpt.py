...
class Head(nn.Module):
    ...
    def forward(self, x):
        ...
        tril = torch.tril(torch.ones(T, T, device=x.device))  # Dynamic causal mask
...
for iter in range(max_epochs):
    ...
    loss.backward()
    torch.nn.utils.clip_grad_norm_(model.parameters(), 1.0)  # Gradient clipping added
    optimizer.step()
...
import dill
with open("model.pkl","wb") as f_obj:
    dill.dump(m,f_obj)